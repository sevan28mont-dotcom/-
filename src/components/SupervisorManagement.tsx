...
export const SupervisorManagement: React.FC<SupervisorManagementProps> = ({
  ...
  const isRecordMatching = (
    ...
    if (typeFilter === 'individual') {
      if (recType === 'group') return false;
    } else if (typeFilter === 'group') {
      if (recType === 'individual') return false;
    }
  ...
  const handleSaveEditedModal = () => {
    ...
    const rec = editingRecordModal.record;
    const hasContent = Boolean(
      (rec.reflection && rec.reflection.trim()) ||
      (rec.transcript && rec.transcript.trim()) ||
      (rec.ideas && rec.ideas.length > 0) ||
      (rec.resources && rec.resources.length > 0)
    );

    if (!hasContent && rec.completed === false) {
      if (onDeleteSupervisionRecord) {
        onDeleteSupervisionRecord(editingRecordModal.mentorId, rec.id);
      }
    } else {
      onUpdateSupervisionRecord(
        ...
        {
          ...rec,
          completed: hasContent ? rec.completed !== false : rec.completed === true,
        }
      );
    }
  ...
  // 计算个体与团体督导自动统计时数 (严格按记录类型与导师类型隔离)
  ...
  mentors.forEach((m) => {
    ...
    (m.records || []).forEach((r) => {
      ...
      const isGroupRecord = r.type === 'group' || (!r.type && m.type === 'group');
      if (isGroupRecord) {
        mGroupCount++;
        mGroupHours += dur;
      } else {
    ...
    const targetQuota = m.totalSupervisions || 0;
    const hasRecords = (m.records || []).length > 0;

    if (!hasRecords) {
      if (m.type === 'group') {
        autoGroupSupervisionHours += targetQuota;
      } else {
        autoIndividualSupervisionHours += targetQuota;
      }
    } else {
      if (mGroupCount > 0) {
        autoGroupSupervisionHours += m.type === 'group' ? Math.max(mGroupHours, targetQuota) : mGroupHours;
      }
      if (mIndivCount > 0) {
        autoIndividualSupervisionHours += m.type !== 'group' ? Math.max(mIndivHours, targetQuota) : mIndivHours;
      }
    }
  });
      ...
      {editingRecordModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-rose-200 dark:border-slate-800 rounded-2xl p-6 max-w-2xl w-full shadow-xl space-y-4 max-h-[90vh] flex flex-col">
            ...
            <div className="flex items-center justify-between gap-2 pt-3 border-t border-rose-100 dark:border-slate-800">
              <button
                type="button"
                onClick={() => {
                  if (onDeleteSupervisionRecord && editingRecordModal) {
                    onDeleteSupervisionRecord(editingRecordModal.mentorId, editingRecordModal.record.id);
                  }
                  setEditingRecordModal(null);
                }}
